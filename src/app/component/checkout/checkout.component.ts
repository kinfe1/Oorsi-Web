...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.route.params.subscribe((param: any) => {
      this.checkoutService.getCheckoutProducts(param["id"]).subscribe(
        (data) => {
  ...
  changeProductCount(cartProduct: CartProduct, changeValue: number) {
    // prevent negative value - check
    if (cartProduct.quantity == 0 && changeValue < 0) {
      return;
    }
    cartProduct.quantity += changeValue;
  }

  actionDeleteCartProduct(cartProduct: CartProduct) {
    this.user.cartProducts = this.user.cartProducts.filter(
